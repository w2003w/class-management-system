...
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'questionnaires') THEN
        CREATE POLICY IF NOT EXISTS "Enable all for questionnaires" ON public.questionnaires FOR ALL TO authenticated USING (true) WITH CHECK (true);
        CREATE POLICY IF NOT EXISTS "Enable read for questionnaires" ON public.questionnaires FOR SELECT TO anon USING (true);
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'questionnaire_records') THEN
        CREATE POLICY IF NOT EXISTS "Enable all for questionnaire_records" ON public.questionnaire_records FOR ALL TO authenticated USING (true) WITH CHECK (true);
    END IF;
END $$;
